...
impl Baseline {
    ...
    /// — writing it straight back to disk under a file lock. Folding the write
    /// in here means callers can't forget it, and the lock keeps a second
    /// writer (another test process) from racing the truncate-and-rewrite.
    pub fn record(
        &mut self,
