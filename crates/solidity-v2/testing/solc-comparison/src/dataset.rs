...
/// Whether the harness rewrites the checked-in baseline (`expected-failures.json`
/// and `pinned-commits.json`) instead of checking against it.
pub fn is_update_mode() -> bool {
    !GitHub::is_running_in_ci()
}
...
/// per version.
///
/// - **Update mode:** rewrite the baseline from the SHAs the tags currently
///   resolve to.
/// - **Checking mode:** every version must have a recorded pin, and each must
///   match — a mismatch means the tag was re-pointed at a different commit, and
///   a missing pin means the baseline is incomplete. Both are hard errors.
...
fn extract_semantic_tests(reader: impl Read, root: &Path) -> Result<String> {
    // The GitHub tarball wraps everything in a single top-level directory
    // (e.g. `solidity-0.8.35/`), so we match the `semanticTests` path anywhere
...
fn strip_to_semantic_tests(path: &Path) -> Option<PathBuf> {
    ...
    let needle = format!("{SEMANTIC_TESTS_PATH}/");
