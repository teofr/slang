...
fn find_shadowed_solidity_declaration(
    ...
    if from_function_signature {
        ...
        // once it exists for the current version/target — `blobhash` is a global
        // only from 0.8.24/Cancun and `erc7201` only from 0.8.35, so before that
        // the name is free (matching solc, which has no such global yet).
