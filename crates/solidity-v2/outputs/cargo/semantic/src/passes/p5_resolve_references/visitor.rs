...
impl Visitor for Pass<'_> {
    ...
    fn enter_contract_definition(&mut self, node: &ir::ContractDefinition) -> bool {
        ...
        // function through its qualified name (eg. `is Base(BaseBase.g())`),
        // which is only legal because the enclosing contract derives from
        // `BaseBase` — so `is_foreign_contract` has to be able to see which
        // contract is being defined, even while names resolve outside of it.
