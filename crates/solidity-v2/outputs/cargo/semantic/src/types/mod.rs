...
pub enum Type {
    ...
    /// It is not a type that can be translated to the EVM; it suspends typing
    /// until more information on how it's used is gathered.
    MetaType(MetaType),
    String(StringType),
    Struct(StructType),
    Tuple(TupleType),
    ...
    /// identifier of a contract, struct or enum. Like [`Type::MetaType`] it is
    /// not representable in the EVM and suspends typing until the usage context
    /// (a cast, a `new` expression, a struct construction, etc.) is known.
