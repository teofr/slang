...
impl<S: Source> CstToIrBuilder<'_, S> {
    ...
    fn build_fallback_as_function(
        ...
        let name = None;
        let parameters = self.build_parameters_declaration(&source.parameters);
        // Non-`external` fallback visibility is rejected by the grammar (see the
        // `FallbackFunctionDefinition` parser-rejection tests), so the IR
        // builder only ever sees the implicit `external` visibility here.
    ...
    fn build_receive_as_function(
        ...
        let name = None;
        let parameters = self.build_parameters_declaration(&source.parameters);
        // Non-`external` receive visibility is rejected by the grammar (see the
        // `ReceiveFunctionDefinition` parser-rejection tests), so the IR builder
        // only ever sees the implicit `external` visibility here.
        let visibility = output::FunctionVisibility::External;
        // Any mutability other than `payable` is rejected by the grammar for a
        // receive function (`view`/`pure` are syntax errors), so a missing
        // specifier can only mean `payable`.
        let mutability = self
            .extract_mutability_specifier(&source.attributes.elements)
