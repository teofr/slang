...
fn type_display(type_: &Type) -> String {
    match type_ {
        ...
        // Meta-types and `super` are expression-only typings (the type of an
        // expression that refers to a type rather than a value). This report
        // only renders the types of value-bearing definitions, so they never
        // reach here.
        Type::MetaType(_) | Type::UserMetaType(_) | Type::Super(_) => {
