...
//! `CompilationBuilder::build()`. That makes it the right subject for the
//! wall-clock benchmarks, which track the end-to-end effect of moving stages
...
pub fn run(project: Input) -> Output {
    ...
    builder.add_files(
        project
            .sources
            .iter()
            .map(|(file_id, contents)| (file_id.as_str().into(), contents.clone())),
    );
...
/// Resolves imports through the project's own remappings, so that benchmarks
