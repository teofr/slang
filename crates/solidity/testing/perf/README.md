...
Valgrind's counters are deterministic, which is exactly what regression detection in CI needs, but
they cannot show the benefit of running work in parallel: moving a work item to another thread costs
the same number of instructions, and Valgrind serializes threads anyway (see
[`gungraun`'s notes on the subject](https://gungraun.github.io/gungraun/latest/html/benchmarks/library_benchmarks/threads_and_subprocesses.html)).
