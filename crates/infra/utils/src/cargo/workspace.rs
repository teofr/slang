...
pub trait CargoWorkspaceCommands {
    ...
    /// (`check`, `clippy`, `rustdoc`). Builds under the `fast-test` profile so
    /// `debug-assertions`/`overflow-checks` stay enabled, matching the code
    /// developers actually run and the test suite.
    #[must_use]
